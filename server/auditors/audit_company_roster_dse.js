...
export async function runDSECompanyListCrossCheck() {
  if (!isScraperEnabled('auditor.external_crosscheck_dse_companylist')) {
    console.log(scraperBlockedMessage('auditor.external_crosscheck_dse_companylist'));
    return { blocked: true };
  }
  ...
  let res;
  try {
    res = await fetchWithRetry(url, { timeout: 45000, attempts: 3, backoffMs: 3000 });
  } catch (err) {
    console.error('[DSE Company List Cross-Check] Live fetch failed:', err.message);
    try {
      await dbRun(`
      ...
      console.error('[DSE Company List Cross-Check] Also failed to save the failure report:', dbErr.message);
    }
  ...
  const dbRows = await dbAll('SELECT symbol, is_active FROM company_list');
  ...
  `, [
    ...
    (liveNotInDb.length === 0 && activeButNotLiveTraded.length === 0) ? 'CERTIFIED_PASSED' : 'AUDIT_FAILED',
    JSON.stringify(summary),
  ]);

  return { summary, liveNotInDb, activeButNotLiveTraded, passed: liveNotInDb.length === 0 && activeButNotLiveTraded.length === 0 };
}
