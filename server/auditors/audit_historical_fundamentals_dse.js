...
import { scrapeCompanyFundamentals } from '../scrapers/scrape_historical_financial_statements.js';
...
// corrupt. Includes historical EPS, NAVPS, DPS, bonus %, P/E, Div Yield, and
// balance sheet items (reserve/surplus, OCI, loans, net income).
const FIELDS = [
  'eps', 'navps', 'dps', 'bonus_pct', 'pe_ratio', 'dividend_yield',
  'reserve_surplus_mn', 'oci_mn', 'short_term_loan_mn', 'long_term_loan_mn', 'net_income_mn'
];
...
/**
 * Cross-checks every fiscal year currently in main DB for `symbols` against a
...
export async function runDSEFundamentalsCrossCheck({ symbols = null } = {}) {
  if (!isScraperEnabled('auditor.external_crosscheck_dse_fundamentals')) {
    console.log(scraperBlockedMessage('auditor.external_crosscheck_dse_fundamentals'));
    return { blocked: true };
  }

  const targetSymbols = symbols || (await dbAll('SELECT DISTINCT symbol FROM fundamentals_history ORDER BY symbol')).map(r => r.symbol);
  ...
  for (let i = 0; i < targetSymbols.length; i++) {
    ...
    const dbRows = await dbAll(
      `SELECT fiscal_year, eps_basic as eps, nav_per_share as navps, dps, bonus_pct,
              pe_ratio, dividend_yield, reserve_surplus_mn, oci_mn,
              short_term_loan_mn, long_term_loan_mn, net_income_mn
